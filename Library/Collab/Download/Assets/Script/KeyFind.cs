...
{
    public static string KeysFind(double keyvalue, string[,] keyindex) //this class takes a key value obtained by using RandomManager.Pick(). It runs it
                                                             //against a reference list of key values (keyindex) and converts it to its equivalent string
                                                             //ex ("1"-->"Grass")
    {
        int loopcount = 0;
        bool looprun = true;
        ...
        while (looprun == true)
        {
            if (keyvalue == Convert.ToDouble(keyindex[0, loopcount]))
            {
