...
public class PlayerPrefsX
{
    ...
    public static double[,] AddTo2DubArray(double[,] targetarray, double key, double value)
    {
        double [,]newarray = new double[2,(targetarray.Length/2)+1];
        for (int intinarray=0;  intinarray< (newarray.Length/2)-1;   intinarray++)
        {
            newarray[0, intinarray] = targetarray[0,intinarray];
            newarray[1, intinarray] = targetarray[1, intinarray];
        }
        newarray[0,(newarray.Length/2)-1]=key;
        newarray[1, (newarray.Length / 2)-1] = value;
    ...
    public static double[] ConvertArrayStringToDouble(string[] targetarray)
    {
        double[] newarray= new double[targetarray.Length];

        for (int loop=0; loop<targetarray.Length;loop++)
        {
            try
            {
                newarray[loop] = Convert.ToDouble(targetarray[loop]);
            }
            catch
            {
    ...
    public static int[] ConvertArrayStringToInt(string[] targetarray)
    {
        int[] newarray = new int[targetarray.Length];

        for (int loop = 0; loop < targetarray.Length; loop++)
        {
            try
            {
                newarray[loop] = Convert.ToInt32(targetarray[loop]);
            }
            catch
            {
                Debug.Log("Error in ConvertArrayStringToInt");
            }
        }
    ...
    public static int[] ConvertStringToIntArray(string targetstring)
    {
        int[] newarray = new int[targetstring.Length];

        for (int loop = 0; loop < targetstring.Length - 1; loop++)
        {
            newarray[loop] = Convert.ToInt32(targetstring.Substring(loop, 1));
        }


        return newarray;
    }
    public static string ConvertIntArrayToString(int[] targetarray)
    {
        string newstring="";
        ...
        for (int loop = 0; loop < targetarray.Length; loop++)
        {
            try
            {
                newstring.Insert(newstring.Length,Convert.ToString(targetarray[loop]));
            }
            catch
            {
                Debug.Log("Error in ConvertIntArrayToString");
            }
        }
    ...
    public static string[] ConvertArrayDoubleToString(double[] targetarray)
    {
        string[] newarray = new string[targetarray.Length];

        for (int loop = 0; loop < targetarray.Length; loop++)
        {
            try
            {
                newarray[loop] = Convert.ToString(targetarray[loop]);
    ...
    public static float[] TwoDimArraytoArray(float[,] targetarray) //converts a 2 dimensional float array into a regular array.
        //[1][2][3]---->[1][2][3][4][5][6]
        //[4][5][6]

    {
        float []newarray = new float[targetarray.Length];
        int newarrayloop = 0;
        while (newarrayloop < targetarray.Length) {
            for (int yloop = 0; yloop < Mathf.Sqrt(targetarray.Length); yloop++)
            {

                for (int xloop = 0; xloop < Mathf.Sqrt(targetarray.Length); xloop++)
                {
                    newarray[newarrayloop] = targetarray[yloop, xloop];
                    newarrayloop++;
                }
