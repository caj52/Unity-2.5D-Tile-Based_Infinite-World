...
public class Tester_Controller : MonoBehaviour {
	...
	public Texture[] armor_premade_textures;
	public Texture[] body_premade_textures;
	...
	void Update()
	{
		//Cubits_Warrior.transform.RotateAround (Vector3.up, rotate_speed * Time.deltaTime);	
	}
...
*/
	...
	//This changes between premade textures since Substance doesnt support WebGL (SAD FACE HERE)
	public void ChangeArmor()
	{
		current_armor++;
		if (current_armor >= armor_premade_textures.Length) 
		{
			current_armor = 0;
		}
		sword.GetComponent<MeshRenderer>().sharedMaterial.SetTexture("_MainTex", armor_premade_textures[current_armor]);
	...
	{
		current_body++;
		if (current_body >= body_premade_textures.Length) 
		{
			current_body = 0;	
		}
		Body_Mesh.sharedMaterial.SetTexture ("_MainTex", body_premade_textures [current_body]);
